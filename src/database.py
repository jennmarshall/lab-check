...
def connect(db):
    try:
        db = sql.connect(db)
        print(f"""INFO: Opened SQLite database with version
        {sql.sqlite_version} successfully.""")
        return db
...
def execute(db, statement):
    try:
        ...
        db.commit()
        print("INFO: SQL statement executed successfully.")
    except sql.OperationalError as e:
...
def create_borrower(db, borrower):
    create_borrower_statement = f'''INSERT INTO borrower (name, number, email)
        VALUES ('{borrower.name}', '{borrower.number}',
        '{borrower.email}');'''
...
def create_equipment(db, equipment):
    create_item_statement = f'''INSERT INTO equipment (name, category,
        dateBorrowed, laboratory, status, borrower) VALUES
        ('{equipment.name}', '{equipment.category}',
        '{equipment.dateBorrowed}', '{equipment.laboratory}',
        '{equipment.status}', '{equipment.borrower}');'''
    execute(db, create_item_statement)
    return db.cursor().lastrowid


def update_borrower(db, id, borrower):
    update_borrower_statement = f'''UPDATE borrower SET name =
        '{borrower.name}', number = '{borrower.number}', email =
        '{borrower.email}' WHERE id = {id};'''
    execute(db, update_borrower_statement)


def update_equipment(db, id, equipment):
    update_item_statement = f'''UPDATE equipment SET name =
    '{equipment.name}', category = '{equipment.category}', dateBorrowed =
    '{equipment.dateBorrowed}', laboratory = '{equipment.laboratory}',
    status = '{equipment.status}', borrower = '{equipment.borrower}'
    WHERE id = {id};'''
